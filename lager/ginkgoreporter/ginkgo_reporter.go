...
// New is a function which returns ginkgo Reporter object
// writer is io.Writer
// name is stdout or ""
func New(writer io.Writer, name string) *GinkgoReporter {
	logger := lager.NewLogger("ginkgo")
	logger.RegisterSink(lager.NewWriterSink(name, writer, lager.DEBUG))
...
func (g *GinkgoReporter) wrappedWithNewlines(f func()) {
	_, errWrite := g.writer.Write([]byte("\n"))
	if errWrite != nil {
		println("wrappedWithNewlines Failed to write log: " + errWrite.Error())
	}
	f()
	_, err := g.writer.Write([]byte("\n"))
	if err != nil {
		println("wrappedWithNewlines Failed to write log: " + err.Error())
	}
}
